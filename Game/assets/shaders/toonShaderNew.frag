...
void main()
{
    ...
    */
    ...
    float toonShininess = mix(128.0, 2.0, roughness); 
    
    float toonSpecularStrength = (roughness > 0.9) ? 0.0 : metallic;
    float specThreshold = mix(0.1, 0.9, roughness);
    ...
    // --- Directional light ---
    {
        ...
        float spec = pow(max(dot(N, H), 0.0), toonShininess);
        float toonSpec = step(specThreshold, spec) * toonSpecularStrength;
