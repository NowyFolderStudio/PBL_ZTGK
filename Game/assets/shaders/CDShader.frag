...
// Dyfrakcja
uniform float u_DiffractionDistance = 2500.0;
uniform float u_DiffractionStrength = 1.0;
uniform bool u_UseDiffraction = false;
...
void main() {
    ...
    if (u_UseDiffraction) {
        ...
        for (int n = 1; n <= 8; n++) {
            float wavelength = abs(sin_thetaL - sin_thetaV) * u_DiffractionDistance / float(n);
            diffractionColor += spectral_zucconi6(wavelength);
        }
