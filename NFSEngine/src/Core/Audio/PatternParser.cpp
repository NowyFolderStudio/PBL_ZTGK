...
namespace NFSEngine {
    int PatternParser::GetNoteValue(const std::string& noteString) {
    ...
    Pattern PatternParser::LoadFromFile(const std::string& filepath) {
        Pattern result;
        std::ifstream file(filepath);

        if (!file.is_open()) {
            NFS_CORE_ERROR("Can't open file: {}", filepath);
            return result;
        }
