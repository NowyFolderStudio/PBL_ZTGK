...
namespace NFSEngine {
	ModelComponent::ModelComponent(GameObject* owner, std::shared_ptr<Model> model, std::shared_ptr<Shader> shader, std::shared_ptr<Texture> texture)
		: Component(owner), p_Model(std::move(model)), p_Shader(std::move(shader)), p_Texture(std::move(texture))
	{
	}
